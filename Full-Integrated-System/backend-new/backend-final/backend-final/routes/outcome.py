...
try:
    ...
    df_dropdown.columns = df_dropdown.columns.str.strip().str.replace(r'[()]+', '',
                                                                      regex=True)  # Use same cleaning as training
    if 'Treatment English Name' not in df_dropdown.columns:
        print(
            f"Warning: 'Treatment English Name' column potentially missing in dropdown dataset. Found: {df_dropdown.columns.tolist()}")
    ...
    if request.method == "POST":
        try:
            ...
            if treatment_col not in filtered_df.columns or symptoms_col not in filtered_df.columns:
                return jsonify(
                    {"error": f"Server configuration error: Missing '{treatment_col}' or '{symptoms_col}'."}), 500
