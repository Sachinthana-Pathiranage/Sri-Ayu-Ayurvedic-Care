...
# --- Configuration ---
output_dir_load = "models" 
try:
    # Get the directory of the current script (app.py)
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    model_dir = os.path.join(BASE_DIR, output_dir_load)
...
def predict_success():
    ...
    if request.method == 'POST':
        try:
            ...
            print("Detailed data type check:", {k: type(v) for k, v in data.items()})
            ...
            # Validate symptom is a list
            if not isinstance(data["symptoms"], list):
                 return jsonify({"error": "Expected 'symptoms' to be a list/array."}), 400
    ...
    if request.method == 'POST':
        try:
            ...
            if not all(field in data for field in required_fields):
                missing = [field for field in required_fields if field not in data]
                return jsonify({"error": f"Missing required fields: {', '.join(missing)}"}), 400
            if not isinstance(data["symptoms"], list): # Expect list
                 return jsonify({"error": "Expected 'symptoms' to be a list/array."}), 400
...
if __name__ == "__main__":
    print(f"Starting Flask server. Models loaded from: {model_dir}")
